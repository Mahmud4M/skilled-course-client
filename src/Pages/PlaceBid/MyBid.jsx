...
const MyBid = () => {
    ...
    const getData = async () => {
        const { data } = await axios('http://localhost:5000/bids')
        setbids(data)
    }
    ...
    const handleDelete = async id => {
        try {
            const { data } = await axios.delete(`http://localhost:5000/bids/${id}`)
            if (data) {
    ...
    return (
        <>
            <div>
                <div>
                    <section className="container px-4 mx-auto font-roboto mt-10">
                        <div className="flex items-center gap-x-3">
                            <h2 className="text-lg font-medium text-gray-800 dark:text-white">Team members</h2>
                            <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400"><span>{bids.length}</span> Bids</span>
                        </div>
                                                    ...
                                                    bids.map(bid => <tr key={bid._id}>
                                                        ...
                                                        <td className="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                                            <div className="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 dark:bg-gray-800">
                                                                ...
                                                                <h2 className="text-sm font-normal text-emerald-500">{new Date(bid.deadline).toLocaleString()}</h2>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{bid.price}</td>
                                                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{bid.clientEmail}</td>
                                                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                            <div className="flex items-center gap-x-2">
                                                                <div className="flex items-center gap-x-6">
                                                                    <Link to={`/updateBid/${bid._id}`}>
                                                        ...
                                                        <td className="px-4 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-300">
                                                            <div>
                                                                <button onClick={() => handleDelete(bid._id)} className="hover:text-red-500 hover:duration-1000"><RiDeleteBin6Fill /></button>
                                                            </div>
                                                        </td>
                                                    </tr>)
                                                }
